...
def test_vacancy_input_field(browser):
...
def test_location_input_field(browser):
...
def test_search_result(browser):
    ...
    assert scraper.search_result(browser), 'Нет результатов поисковой выдачи'


def test_scraper_class(browser):
